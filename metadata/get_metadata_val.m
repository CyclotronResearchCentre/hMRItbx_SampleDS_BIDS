...
%
% BIDS COMPATIBILITY
% The following rules for metadata storage must be followed for BIDS
% compliance:
% - BIDS acquisition parameters must be stored at the root of the
%   structure saved in the JSON side-car file.
% - BIDS naming convention of the fields must be respected.
% - Parameters must be expressed in the International System of Units (SI)
% For a more general usage of the metadata library, the get_metadata_val
% function must retrieve the parameter values according to the following
% order of priority:
% - if no metadata at all, search the NIfTI header description field (only
%   useful for SPM-converted images where TR, TE and FA are stored in the
%   description)
% - if metadata available:
%       1. search the BIDS fields only
%       2. search the acqpar field (containing the whole original DICOM
...
% [parValue, parLocation] = get_metadata_val(filenam, inParName,opt)
% parValue = get_metadata_val(filenam, 'RepetitionTime')
% parValue = get_metadata_val(filenam, 'RepetitionTimeExcitation','BIDS')
%
% ARGUMENTS
...
%   Siemens-only:
%    = 'RepetitionTime' TR [s] = inter-volume interval for fMRI
%    = 'RepetitionTimes' TR [s]
%    = 'RepetitionTimeExcitation' TR [s] = acquisition TR for sMRI
%    = 'RepetitionTimesExcitation' TR [s]
%    = 'EchoTime' TE(s) [s]
%    = 'EchoTimes' TE(s) [s]
%    = 'FlipAngle' [deg]
%    = 'ProtocolName'
...
%    - 'PELinesPF'
%    - 'PELinesPAT'
%    - 'PhaseEncodingDirectionSign' A>>P & R>>L = 1; P>>A & L>>R = 0.
%    = 'PhaseEncodingDirection' A>>P/P>>A = 'COL' & R>>L/L>>R = 'ROW'
%    - 'NumberOfMeasurements'
%    - 'NumberOfSlices'
%    - 'epiReadoutDuration' [s]
...
% - opt can be set to 'BIDS' to limit the search for the metadata in the
%   BIDS fields of the metadta structure. Otherwise, the search starts with
%   the BIDS fields then follows in the acqpar field.
%
...
if nargin==3
    opt = varargin{3};
    if strcmpi(opt,'BIDS')
        fprintf(1,'\nWARNING: limiting the metadata search to the BIDS fields!');
    end
else
    opt = '';
end
...
if ~isstruct(mstruc)
    ...
    if ischar(varargin{1}) && strcmp(spm_file(varargin{1},'ext'),'nii')
        ...
        if ~isempty(tmp)
            ...
            switch inParName
                case {'RepetitionTime','RepetitionTimeExcitation'} % [s]
                    cRes = 1;
                    parLocation{cRes} = 'NiftiDescriptionField';
                    parValue{cRes} = p.tr*0.001;
                    fprintf(1,['Parameter available in NIFTI description field:\n\tRepetitionTime = %5.5f s\n' ...
                        'Depending on the sequence, the RepetitionTime might be the time interval \n' ...
                        'between two succesive excitations (RepetitionTimeExcitation) or the time \n' ...
                        'required to acquire a whole volume (typically the RepetitionTime used in \n' ...
                        'fMRI). Make sure the retrieved value is coherent with your expectation, \n' ...
                        'whichever type of TR you attempted to retrieve.\n'], parValue{1});
                    
                case 'EchoTime'
                    cRes = 1;
                    parLocation{cRes} = 'NiftiDescriptionField';
                    parValue{cRes} = p.te*0.001;
    ...
    % first search the BIDS fields only (if available)
    mstrucBIDS = mstruc;
    if isfield(mstrucBIDS,'history')
        mstrucBIDS = rmfield(mstrucBIDS,'history');
    end
    if isfield(mstrucBIDS,'acqpar')
        mstrucBIDS = rmfield(mstrucBIDS,'acqpar');
    end
    % search the structure cleared from 'history' and 'acpar' fields. This
    % might well be a structure with no fields in which case the result
    % will be empty. The parameter might also not be present in the BIDS
    % part of the metadata, in which case, the result will be empty as
    % well. In both these cases, the search is extended to the 'acqpar'
    % and 'history' fields (if any).
    [nFieldFound, fieldList] = find_field_name(mstrucBIDS, inParName, 'caseSens','sensitive','matchType','exact');
    if nFieldFound
        [val,nam] = get_val_nam_list(mstrucBIDS, nFieldFound, fieldList);
        cRes = 1;
        parLocation{cRes} = nam{1};
        parValue{cRes} = val{1};
    elseif ~strcmpi(opt,'BIDS')
        % extends the search to the 'acqpar' and 'history' fields if any
        switch inParName
            case 'RepetitionTime' % [s]
            ...
            case 'EchoTimes' % [s]
                ...
                % alTE is a Siemens-specific field which holds an array of
                % TE values (stored in us) rather than a single value
                % -> convenient if several TEs used for a given sequence
                % (e.g. multiecho sequences). If not available (GE or
                % Philips), let's get the standard DICOM field EchoTime
                % instead (stored in ms):
                if nFieldFound
                    [val,nam] = get_val_nam_list(mstruc, nFieldFound, fieldList);
                    cRes = 1;
                    parLocation{cRes} = nam{1};
            ...
            case 'epiReadoutDuration' % [s]
                ...
                if nFieldFound
                    ...
                    if strfind(lower(valPROT),'b1map')
                        ...
                        nFieldFound = 1;
                        switch lower(valSEQ) % expressed in [s]
                            case 'b1v2d3d2' % 540 us - Prisma
                                EchoSpacing = (2*140+260)*1e-6;
            ...
            case 'B1mapMixingTime' % [s] for al_B1mapping - version dependent!!
                valSEQ = get_metadata_val(mstruc, 'SequenceName');
                valPROT = get_metadata_val(mstruc, 'ProtocolName');
                valMODELNAME = get_metadata_val(mstruc, 'ManufacturerModelName');
                [nFieldFound, fieldList] = find_field_name(mstruc, 'sWipMemBlock','caseSens','insensitive','matchType','exact');
                if nFieldFound
                    ...
                    if index
                        ...
                        parValue{cRes} = val{1}.alFree(index)*0.000001; % in s
                    end
                end
